...
class Classification(BaseModel):
    """
        定义一个Pydantic的数据模型，没来需要根据这个类型来完成文本分类
    """
    # 文本的情感倾向，预期为字符串类型
    sentiment: str = Field(..., enum=['happy', 'neutral', 'sad'], description="文本的情感")

    # 文本的攻击性，预期为1到10的整数
    aggressiveness: int = Field(..., enum=[1, 2, 3, 4, 5], description="描述文本的攻击性，数字越大表示攻击性越强")

    # 文本使用的语言，预期为字符串类型
    language: str = Field(..., enum=['English', "spanish", "french", "中文", "italian"], description="文本使用的语言")
